...
const getClientsData = async () => {
  try {
    const data = await baseService.getData(DATABASE_TABLE_KEYS.clients);
...
/**
 *
 * @param {Client} clientsData
 * @returns {Response}
 */
const updateClientsData = async (clientsData) => {
  try {
    await baseService.updateObjectData(
      clientsData,
      DATABASE_TABLE_KEYS.clients
    );
